...
def get_historical_data(symbol=SYMBOL, interval=INTERVALS, limit=LIMIT):
...
if __name__ == "__main__":
    for symbol in SYMBOL:
        for interval in INTERVALS:
            ...
            try:
                df = get_historical_data(symbol=symbol, interval=interval) # Utiliser le symbole par défaut
                ...
                save_to_postgres(df, symbol, interval) # Sauvegarder dans PostgreSQL
                print(f"Données sauvegardées pour le symbole: {symbol} - {interval}")
            except Exception as e:
                print(f"Erreur lors de la récupération des données pour le symbole {symbol}: {e}")
